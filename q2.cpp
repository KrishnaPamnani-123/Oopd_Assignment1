...
double calculateSellingPrice2(const vector<YearData>& data, int purchaseYear, int sellingYear, double initialPrice) {
    double sellingPrice = initialPrice;
    for (const auto& yearData : data) {
        if (yearData.year > purchaseYear && yearData.year <= sellingYear) {
            sellingPrice *= (1 + (yearData.growthPrice) / 100);
        }
    }
    return sellingPrice;
}
double calculateLTCG2(double sellingPrice, double initialPrice) {
    double ans = sellingPrice - initialPrice;
    if (ans < 0){return 0;}
    return (sellingPrice - initialPrice) * 0.125;
}
...
int main() {
    ...
    double sellingPrice2 = calculateSellingPrice2(data, purchaseYear, sellingYear, initialPrice);
    double LTCG2 = calculateLTCG2(sellingPrice2, initialPrice);

    cout << "\nNew Scheme (12.5% tax without inflation adjustment):" << endl;
    cout << fixed << setprecision(2);
    cout << "Estimated Selling Price: Rs " << sellingPrice2 << endl;
    cout << "Long-term Capital Gains Tax (LTCG): Rs " << LTCG2 << endl;

    double difference =LTCG2 - LTCG;
    cout << "\nComparison:" << endl;
    if (difference > 0) {
        cout << "The new scheme leads to higher taxes by Rs. " << difference << endl;
    } else if (difference < 0) {
        cout << "The old scheme leads to higher taxes by Rs. " << -difference << endl;
    } else {
        cout << "Both schemes result in the same tax amount." << endl;
    }
